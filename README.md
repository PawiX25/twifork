...
- **`GuestClient` does not work, and no fix here changes that.** The `User-Agent` header and the defensive user parsing are in place ([#402](https://github.com/d60/twikit/issues/402), [#385](https://github.com/d60/twikit/issues/385)), so `activate()` itself succeeds - but that is where it ends. Verified against live X: a logged-out request only ever gets a ~34 KB page shell with no webpack manifest, so the `x-client-transaction-id` handshake cannot complete and every call fails with `InvalidSession`. Guest access is closed on X's side; use cookies. ([#192](https://github.com/d60/twikit/issues/192))
